...
TEST(AccessStrategyTest, Pair) {

	AccessStrategy<int> csf(2);
...
TEST(AccessStrategyTest, FewPairs) {
...
TEST(AccessStrategyTest, Iterators) {
	AccessStrategy<int> cs(2);
...
TEST(AccessStrategyTest, FewPairsIterators) {
	AccessStrategy<int> cs(1);
	HashTable<int, int> ht(cs);
	for (int i = 0; i < 4; i++)
	{
		ht.insert(i, i + 10);
